...
for s in scripts:
    ...
    verdict = []
    if orig_missing: verdict.append("前身不符(原始字节找不到)")
    if already: verdict.append("已装/部分已装")
    if hashes: verdict.append(f"含{hashes}个哈希门")
    if not verdict: verdict.append("前身匹配")
    rows.append((s.name, ",".join(fixes) or "无洞", " / ".join(verdict), len(orig), len(patched)))
print(f"{'script':<50}{'洞需求':<24}{'判定'}")
