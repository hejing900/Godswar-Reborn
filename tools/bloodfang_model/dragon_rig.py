"""Original 17-bone juvenile dragon rig and six authored animation actions."""
import math
import bpy
from mathutils import Vector, Quaternion
from dragon_geometry import TAIL_POINTS


def create_dragon_rig(mesh):
    data=bpy.data.armatures.new("Bloodfang_DragonSkeleton")
    rig=bpy.data.objects.new("Bloodfang_DragonRig",data)
    bpy.context.collection.objects.link(rig)
    bpy.context.view_layer.objects.active=rig;rig.select_set(True)
    bpy.ops.object.mode_set(mode="EDIT")
    layout=[("root",None,(0,0,0),(0,.5,0)),
            ("spine","root",(0,0,.66),(0,0,1.75)),
            ("neck","spine",(0,.02,1.72),(0,0,2.32)),
            ("head","neck",(0,0,2.30),(0,0,2.98)),
            ("jaw","head",(0,-.19,2.21),(0,-.93,2.20)),
            ("tail.01","spine",TAIL_POINTS[0],TAIL_POINTS[2]),
            ("tail.02","tail.01",TAIL_POINTS[2],TAIL_POINTS[3]),
            ("tail.03","tail.02",TAIL_POINTS[3],TAIL_POINTS[5]),
            ("tail.04","tail.03",TAIL_POINTS[5],TAIL_POINTS[6])]
    for s,suffix in ((1,"L"),(-1,"R")):
        layout.extend([
            ("wing."+suffix,"spine",(s*.43,.22,1.94),(s*1.61,.27,2.72)),
            ("wingtip."+suffix,"wing."+suffix,(s*1.61,.27,2.72),(s*2.80,.37,2.42)),
            ("arm."+suffix,"spine",(s*.37,-.025,1.73),(s*.60,-.56,1.12)),
            ("foot."+suffix,"spine",(s*.44,.08,.82),(s*.60,-.28,.20)),
        ])
...
def build_dragon_actions(rig):
    ...
    for name,last in [("Idle",48),("Move",24),("Attack",30),("Death",42),("Angry",36),("Happy",36)]:
        rig.animation_data_create();action=bpy.data.actions.new("Bloodfang_Dragon_"+name)
        ...
        for frame in sorted(set([1,last]+list(range(1,last+1,3)))):
            ...
            if name=="Idle":
                root.location.z=.07+.035*math.sin(phase)
                flap=.07*math.sin(phase*2);tip=.04*math.sin(phase*2-.5)
                neck.rotation_euler.x=.025*math.sin(phase)
                head.rotation_euler.z=.025*math.sin(phase)
                arms=.03*math.sin(phase)
            elif name=="Move":
                root.location.z=.21+.12*math.sin(phase*2-.5)
                spine.rotation_euler.x=.14;neck.rotation_euler.x=-.10
                flap=.34*math.sin(phase*2);tip=.14*math.sin(phase*2-.5)
                arms=-.24;feet=.24;tail=.11*math.sin(phase-.5)
            elif name=="Attack":
                strike=max(0,math.sin(math.pi*min(1,max(0,(t-.15)/.68))))
                root.location.y=-.60*strike;root.location.z=.06+.09*strike
                spine.rotation_euler.x=.19*strike;neck.rotation_euler.x=.17*strike
                head.rotation_euler.x=-.12*strike
                # The jaw's local X runs toward -X: negative rotation opens it.
                jaw.rotation_euler.x=-.52*strike
                flap=-.24*strike;tip=-.10*strike;arms=-.35*strike
                tail=-.13*strike
            elif name=="Angry":
                bristle=math.sin(math.pi*t)
                root.location.z=.07+.015*math.sin(phase*4)
                ...
                root.location.z=.07+.20*(1-math.cos(phase*2))*.5
