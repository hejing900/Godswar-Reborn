...
function Invoke-AvatarPreloadPatch {
    ...
    # marked executable first (three header bytes), and the two QuestView guards
    # are installed on top, so the whole-file hash differs even though every
    # structural check below - the V3 guard hooks and caves, this patch's hook
    # bytes, its continuations, and both empty caves - still matches the audited
    # build. The after hash was computed from this patch's own four write ranges
    # and confirmed by the 206-byte mutation assertion below.
    $priorSha256 =
