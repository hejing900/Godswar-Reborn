#!/usr/bin/env python3
"""Preview, apply, or verify Ascension Core artwork and presentation."""
...
from ascension_core_icons import build_plan
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--asset-root", type=Path,
                        default=Path(__file__).resolve().parents[1] / "assets/ascension-core")
    ...
    parser.add_argument("--repository-source", action="store_true",
                        help="Patch only tracked English client metadata in --client-root")
    ...
    try:
        ...
        if args.mode == "apply":
            ...
            if any(c.changed for c in build_plan(root, assets, repository_source=args.repository_source)):
                raise InstallError("Post-install Ascension Core verification differs")
        elif args.mode == "verify":
            if any(c.changed for c in plan):
                raise InstallError("Ascension Core presentation requires installation")
        ...
        print(f"Ascension Core installation failed: {error}", file=sys.stderr)
        return 1
