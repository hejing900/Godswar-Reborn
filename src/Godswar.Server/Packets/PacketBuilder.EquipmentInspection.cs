...
<<<<<<< HEAD
    ...
    /// <summary>
    /// The same bag-item snapshot addressed to an authoritative world object.
    /// Bag activation (opcode 10051) is answered for the using player, whose
    /// world object ID is not the client-local constant.
    /// </summary>
...
<<<<<<< HEAD
    ...
    /// <summary>
    /// Patches a 92-byte item snapshot into the bag activation reply shape:
    /// zero at +8, the packed bag page at +12 and the index inside it at +14.
    /// Captured S2C 10051 replies use exactly this shape (ids 183610 and
    /// 145494).
    /// </summary>
    internal static void WriteSnapshotBagPosition(byte[] packet, int bagSlot)
    {
        BinaryPrimitives.WriteUInt32LittleEndian(packet.AsSpan(8, 4), 0);
