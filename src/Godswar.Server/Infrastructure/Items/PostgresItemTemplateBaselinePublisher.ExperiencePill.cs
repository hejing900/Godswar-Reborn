...
{
    private static async Task<IReadOnlyList<ItemTemplateDefinition>> AppendExperiencePillAsync(
        ...
        IReadOnlyList<ItemTemplateDefinition> prior, CancellationToken token)
    {
        ...
        foreach (var expected in await ReadCanonicalExperiencePillAsync(connection, transaction, token))
        {
            if (definitions.TryGetValue(expected.Id, out var actual) && !DefinitionsEquivalent(actual, expected))
                throw new InvalidOperationException($"Published EXP Pill {expected.Id} conflicts with native content.");
            definitions[expected.Id] = expected;
        }
        return definitions.Values.OrderBy(item => item.Id).ToArray();
    }

    private static async Task<bool> PublishedExperiencePillIsCompleteAsync(
        NpgsqlConnection connection, NpgsqlTransaction transaction, string revision, CancellationToken token)
    {
        var expected = await ReadCanonicalExperiencePillAsync(connection, transaction, token);
        ...
        await using var read = new NpgsqlCommand("""
            ...
            WHERE revision=@revision AND id = 4174 ORDER BY id;
    ...
    private static async Task<IReadOnlyList<ItemTemplateDefinition>> ReadCanonicalExperiencePillAsync(
        NpgsqlConnection connection, NpgsqlTransaction transaction, CancellationToken token)
    {
        var seeds = ExperiencePillItemContentBaseline.ItemTemplates;
    ...
    private static async Task EnsureExperiencePillMutableCompatibilityAsync(
        NpgsqlConnection connection, NpgsqlTransaction transaction, string revision, CancellationToken token)
    {
        foreach (var expected in await ReadCanonicalExperiencePillAsync(connection, transaction, token))
        {
                ...
                """, connection, transaction))
            {
                ...
                if (await reader.ReadAsync(token) && !DefinitionsEquivalent(ReadDefinition(reader), expected))
                    throw new InvalidOperationException($"Mutable EXP Pill {expected.Id} conflicts with native content.");
            }
