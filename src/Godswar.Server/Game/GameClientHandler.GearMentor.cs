        ...
        if (npcDelta.Entering.Count > 0)
        {
            ...
            foreach (var npc in projected)
            {
                QuestFrameTrace.Append(
                    $"[npc-out] reason={reason} map={_character.CurrentMap} " +
                    $"object={npc.ObjectId} key={npc.NpcKey} " +
                    $"template={npc.TemplateKey} appearance={npc.AppearanceType} " +
                    $"x={npc.X:0.##} z={npc.Z:0.##} " +
                    $"table77={npc.Detail10077.Length} " +
                    $"table80={npc.Detail10080.Length}",
                    []);
            }
