...
{
    private static async Task AssertV19DeltaAsync(
        NpgsqlDataSource dataSource)
    {
        ...
        Check.True(await reader.ReadAsync(), "canonical V19 release exists");
        var expectedCounts = new[]
        {
            NpcDialogueBaselineV19.ExpectedTextCount,
            NpcDialogueBaselineV19.ExpectedProfileCount,
            NpcDialogueBaselineV19.ExpectedRouteCount,
            NpcDialogueBaselineV19.ExpectedMenuEntryCount
        };
        Check.True(
            reader.GetString(0) ==
                NpcDialogueBaselineV19.ExpectedSpawnRevision &&
            reader.GetString(5) == NpcDialogueBaselineV19.Source,
            "V19 declares its canonical V6 spawn dependency and source");
        for (var index = 0; index < expectedCounts.Length; index++)
        {
            Check.Equal(
                expectedCounts[index],
                reader.GetInt32(index + 1),
                $"V19 declared count {index}");
            Check.Equal(
                expectedCounts[index],
                reader.GetInt32(index + 6),
                $"V19 stored count {index}");
        }

        await reader.CloseAsync();
        await AssertV19ArenaRosterAsync(dataSource);
    }

    private static async Task AssertV15ToV19DeltaAsync(
        NpgsqlDataSource dataSource)
    {
        await using var command = dataSource.CreateCommand(
            """
            SELECT
              ...
              (SELECT COUNT(*)::integer
               ...
               WHERE predecessor.revision = @v15
                 AND current_profile.revision = @v19),
              (SELECT COUNT(*)::integer
               ...
               WHERE predecessor.revision = @v15
                 AND current_entry.revision = @v19),
              (SELECT COUNT(*)::integer
               ...
               WHERE predecessor.revision = @v15
                 AND current_binding.revision = @v19),
              (SELECT COUNT(*)::integer
               ...
               WHERE current_text.revision = @v19
                 AND predecessor.npc_key IS NULL);
            """);
        AddV15AndV19Revisions(command);
        await using var reader = await command.ExecuteReaderAsync();
        Check.True(
            ...
            reader.GetInt32(4) == 3,
            "V19 changes only two inherited Arena descriptions and adds " +
            "three native support texts to V15");
    }

    private static async Task AssertV19ArenaRosterAsync(
        NpgsqlDataSource dataSource)
    {
        await using var command = dataSource.CreateCommand(
            """
        ...
        await using var reader = await command.ExecuteReaderAsync();
        var expected = NpcContentBaselineV6.LoadDefinitions()
            .Where(static npc =>
        ...
        {
            Check.True(index < expected.Length, "V19 has no extra Arena NPC");
            var actor = expected[index++];
            var routeCount = DuelArenaTransporterProtocol.IsEndpoint(
                actor.NpcKey,
            ...
            Check.True(
                ...
                $"V19 persists exact Arena actor {actor.NpcKey}");
        }

        Check.Equal(5, index, "V19 persists all five Arena actors");
    }

    private static void AddV15AndV19Revisions(NpgsqlCommand command)
    {
        command.Parameters.AddWithValue(
            "v15",
            NpgsqlDbType.Varchar,
