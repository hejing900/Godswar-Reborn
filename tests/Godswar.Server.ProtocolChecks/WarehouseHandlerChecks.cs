...
<<<<<<< HEAD
    private static async Task CheckCommittedWithdrawAsync()
    {
        var beforeCharacter = CharacterSnapshot(
            GameDefaults.EmptyKitBag,
            BeforeInventoryRevision,
            "warehouse-handler-withdraw-before");
        var afterCharacter = CharacterSnapshot(
            KitBagSlots.SetSlot(
                GameDefaults.EmptyKitBag,
                AutoResolvedKitBagSlot,
                StorageKey.ToCompactString()),
            AfterInventoryRevision,
            "warehouse-handler-withdraw-after");
        var beforeWarehouse = WarehouseSnapshot(
            BeforeInventoryRevision,
            containsKey: true);
        var afterWarehouse = WarehouseSnapshot(
        ...
        // The automatic (right-click) withdrawal request carries a -1 bag
        // sentinel, so the acknowledgement must name the cell the executor
        // actually resolved or the client cannot settle the transfer.
        await InvokeAsync(fixture.Handler, CreateWithdrawRequest());

        var packets = fixture.ReadPackets().Skip(openPacketCount).ToArray();
        var sourceClearIndex = FindOpcode(packets, Opcodes.WarehouseTransfer);
        var bagIndex = FindOpcode(packets, 0x2731);
        var snapshotIndex = FindOpcode(packets, Opcodes.WarehouseSnapshot);
        Check.True(
            sourceClearIndex >= 0 &&
            packets.Count(packet =>
                ReadOpcode(packet) == Opcodes.WarehouseTransfer) == 1 &&
            IsWarehouseSourceClear(
                packets[sourceClearIndex],
                WarehouseSlot,
                AutoResolvedKitBagSlot) &&
            packets.Count(packet =>
                ReadOpcode(packet) == 0x2744) == 0 &&
            packets.Count(packet =>
                ReadOpcode(packet) == Opcodes.WarehouseSnapshot) == 4,
            "withdrawal acknowledges the vacated warehouse cell exactly once " +
            "and names the resolved destination bag cell");
        Check.True(
            bagIndex > sourceClearIndex &&
            snapshotIndex > sourceClearIndex,
            "the warehouse source clear precedes both inventory projections");
        Check.True(
            executor.ReplayCount == 1 &&
            executor.ExecuteCount == 1 &&
            executor.Envelope?.Command is { } command &&
            command.Operation == WarehouseTransferOperation.Withdraw &&
            command.WarehouseSlot == WarehouseSlot &&
            command.ExpectedSourceCompactItemState ==
                StorageKey.ToCompactString() &&
            command.ExpectedDestinationCompactItemState == "[]",
            "withdrawal executes against the authoritative warehouse cell");
        Check.True(
            fixture.Character.KitBag ==
                KitBagSlots.SetSlot(
                    ...
                    AutoResolvedKitBagSlot,
                    StorageKey.ToCompactString()),
            "withdrawal hydrates the resolved destination bag cell");

        var secure = fixture.Transport.CommandResults.Single();
        Check.True(
            secure.CommandFamily == (ushort)CommandFamily.WarehouseTransfer &&
            secure.ResultCode ==
                (int)WarehouseTransferResultStatus.Withdrawn &&
            secure.Disposition == SecureLegacyCommandDisposition.Applied,
...
<<<<<<< HEAD
    private static bool IsWarehouseSourceClear(
        ...
        int kitBagSlot)
    {
        var page = Math.DivRem(kitBagSlot, 24, out var cell);
        return packet.Length == WarehouseWireProtocol.TransferPacketBytes &&
            BinaryPrimitives.ReadInt16LittleEndian(packet.AsSpan(4)) ==
                checked((short)warehouseSlot) &&
            BinaryPrimitives.ReadInt16LittleEndian(packet.AsSpan(6)) ==
                checked((short)page) &&
            BinaryPrimitives.ReadInt16LittleEndian(packet.AsSpan(8)) ==
                checked((short)cell) &&
            packet[16] == 0 &&
            BinaryPrimitives.ReadUInt16LittleEndian(packet.AsSpan(18)) ==
