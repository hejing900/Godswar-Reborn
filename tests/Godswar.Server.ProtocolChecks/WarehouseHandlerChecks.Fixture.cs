...
<<<<<<< HEAD
    ...
    /// deliberately neither the request sentinel (-1) nor <see cref="KitBagSlot"/>,
    /// so an acknowledgement that echoes the request instead of the receipt
...
<<<<<<< HEAD
    private static WarehouseTransferExecutionReceipt WithdrawReceipt(
        ...
        DestinationWarehouseSlot: -1,
        ActualWarehouseSlot: warehouseSlot,
        ActualKitBagSlot: AutoResolvedKitBagSlot,
        ...
        [new WarehouseItemMutation(
            ...
            ItemId: 4102,
            WarehouseInventoryLocation.Warehouse,
            BeforeSlot: warehouseSlot,
            BeforeStack: 1,
            WarehouseInventoryLocation.KitBag,
            AfterSlot: KitBagSlot,
            AfterStack: 1)],
...
<<<<<<< HEAD
        ...
        int warehouseSlot = WarehouseSlot)
    {
        ...
        BinaryPrimitives.WriteInt16LittleEndian(
            bytes.AsSpan(4),
            checked((short)warehouseSlot));
        // Stock withdrawal sends both target fields as -1: the client lets
        // the server choose the destination bag cell.
        BinaryPrimitives.WriteInt16LittleEndian(bytes.AsSpan(6), -1);
        BinaryPrimitives.WriteInt16LittleEndian(bytes.AsSpan(8), -1);
        bytes[16] = 0;
        BinaryPrimitives.WriteUInt16LittleEndian(
            bytes.AsSpan(18),
            (ushort)WarehouseStorageType.Normal);
