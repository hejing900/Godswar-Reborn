...
- Client revive opcode `10028` (captured 2026-09-15) is exactly 12 bytes: local
  ...
  contract. `10019` carried the same body in the earlier lineage captures and
  is still accepted as `Opcodes.ReviveLegacy`.
- A free revive returns the dead character to the map it died on, at that map's
  own revive point (`ReviveLandingCatalog`). The capture-proven Athens city
  point is x = `20`, z = `-100`; a map without a captured point keeps the camp
